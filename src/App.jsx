...
const YoloDetectionPage = lazy(() => import('./components/YoloDetectionPage.jsx'));

const ADMIN_USERNAME = 'admin';
const ADMIN_PASSWORD = 'admin123';
const STORAGE_KEY = 'iot-dashboard-api-config-v1';
const YOLO_STORAGE_KEY = 'airnav-yolo-detection-config-v2';
...
const navItems = [
  ...
  { id: 'yolo', label: 'Deteksi YOLO', icon: '🦺' },
...
function normalizeCameraSource(value) {
  ...
  const hostPort = slashIndex >= 0 ? text.slice(0, slashIndex) : text;
  const path = slashIndex >= 0 ? text.slice(slashIndex) : '';
...
function normalizeYoloConfig(config = {}) {
  const rawYoloEnabled = config.yoloEnabled ?? config.yolo_enabled ?? DEFAULT_YOLO_CONFIG.yoloEnabled;
  return {
    ...DEFAULT_YOLO_CONFIG,
    ...config,
    backendUrl: normalizeBackendUrl(config.backendUrl),
    cameraSource: normalizeCameraSource(config.cameraSource),
    modelPath: String(config.modelPath || DEFAULT_YOLO_CONFIG.modelPath).trim() || DEFAULT_YOLO_CONFIG.modelPath,
    yoloEnabled: !(rawYoloEnabled === false || rawYoloEnabled === 'false' || rawYoloEnabled === '0'),
    confidence: String(config.confidence || DEFAULT_YOLO_CONFIG.confidence).trim(),
    imageSize: String(config.imageSize || DEFAULT_YOLO_CONFIG.imageSize).trim(),
    inferEvery: String(config.inferEvery || DEFAULT_YOLO_CONFIG.inferEvery).trim(),
    jpegQuality: String(config.jpegQuality || DEFAULT_YOLO_CONFIG.jpegQuality).trim(),
  };
}

function yoloApiUrl(baseUrl, path) {
...
function DashboardPage({ temperature, rfid, ats, onNavigate }) {
  ...
  const activeSystems = [temperature.length, rfid.length, ats.length, 1].filter(Boolean).length;
    ...
    <div>
      ...
      <section className="relative overflow-hidden rounded-[2rem] border border-cyan-100 bg-slate-950 p-6 text-white shadow-2xl shadow-cyan-950/20 lg:p-8">
        ...
        <div className="relative grid gap-8 xl:grid-cols-[1.15fr_0.85fr] xl:items-center">
          <div>
            <div className="mb-5 flex flex-wrap items-center gap-2"><span className="rounded-full border border-cyan-300/30 bg-cyan-300/10 px-3 py-1 text-xs font-bold uppercase tracking-wider text-cyan-100">Admin Command Center</span><span className="rounded-full border border-white/20 bg-white/10 px-3 py-1 text-xs font-bold uppercase tracking-wider text-slate-100">{activeSystems}/4 Sistem Online</span><Badge tone={riskTone}>{riskLabel}</Badge></div>
            ...
            <div className="mt-6 flex flex-col gap-3 sm:flex-row">
              ...
              <button onClick={() => onNavigate('yolo')} className="rounded-2xl border border-emerald-200/30 bg-emerald-400/15 px-4 py-3 text-sm font-black text-emerald-50 transition hover:bg-emerald-400/25">Deteksi YOLO</button>
            </div>
          </div>
          <div className="rounded-[1.7rem] border border-white/10 bg-white/10 p-5 backdrop-blur">
            ...
            <div className="mt-6 space-y-4">
              {[
                ...
                ['Deteksi APD YOLO', 'ESP32-S3 AI Camera', 'Helm & Rompi AirNav', 'yolo'],
...
function TemperaturePage({ data, onDownload, canDownload = false }) {
  ...
  return <div><PageHeader title="Monitoring Suhu" description="Pantau suhu dan kelembaban ruang secara real-time dari sensor DHT11.">{canDownload && <DownloadButton disabled={data.length === 0} onClick={onDownload || (() => downloadCsv(`data-suhu-${new Date().toISOString().slice(0, 10)}.csv`, temperatureExportColumns, toTemperatureExportRows(tableRows)))} />}</PageHeader><div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><StatCard title="Suhu Terkini" value={latest.suhu ?? '-'} suffix="°C" status={latest.keterangan} icon="🌡️" /><StatCard title="Kelembaban" value={latest.kelembaban ?? '-'} suffix="%" icon="💧" tone="blue" /><StatCard title="Rata-rata Suhu" value={avgTemp} suffix="°C" icon="📊" tone="purple" /><StatCard title="Status" value={latest.keterangan || '-'} icon={temperatureVisual.icon} tone={temperatureVisual.tone} effect={temperatureVisual.effect} /></div><div className="mt-6"><ChartCard title="Grafik Suhu dan Kelembaban" description="Riwayat pembacaan sensor per jam."><LazyLineChart data={data.slice(-120)} lines={[{ dataKey: 'suhu', name: 'Suhu °C', stroke: '#0891b2', dot: false }, { dataKey: 'kelembaban', name: 'Kelembaban %', stroke: '#2563eb', dot: false }]} /></ChartCard></div><div className="mt-6"><DataTable columns={columns} rows={tableRows} emptyMessage="Belum ada data suhu." /></div></div>;
}

function RfidPage({ data, onDownload, canDownload = false }) {
  ...
  return (
    <div>
      <PageHeader title="RFID Alat" description="Monitoring peminjaman dan pengembalian alat berbasis kartu RFID.">
        {canDownload && <DownloadButton disabled={data.length === 0} onClick={onDownload || (() => downloadCsv(`data-rfid-${new Date().toISOString().slice(0, 10)}.csv`, rfidExportColumns, toRfidExportRows(data)))} />}
      </PageHeader>
...
function AtsPage({ data, onDownload, canDownload = false }) {
  ...
  return (
    <div>
      <PageHeader title="ATS PLN / Generator" description="Parameter mengikuti kolom log Sheet: tegangan/arus/watt per sumber, status PLN & Generator (YA/TIDAK), sumber aktif, relay, dan keterangan.">
        {canDownload && <DownloadButton disabled={data.length === 0} onClick={onDownload || (() => downloadCsv(`data-ats-${new Date().toISOString().slice(0, 10)}.csv`, atsExportColumns, toAtsExportRows(data)))} />}
      </PageHeader>
    ...
    <div>
      ...
      <Card className="mt-6 bg-slate-950 text-white">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
          <div>
            <h2 className="text-lg font-black">Pengaturan Kamera & YOLO</h2>
            <p className="mt-1 max-w-3xl text-sm leading-6 text-slate-300">Masukkan IP kamera, URL stream HTTP/MJPEG, atau RTSP. Backend API YOLO akan mencoba endpoint stream umum otomatis, jadi kamera tidak harus memakai firmware ESP32 khusus.</p>
          </div>
          <Badge tone={String(yoloMessage || '').toLowerCase().includes('gagal') || String(yoloMessage || '').toLowerCase().includes('belum') ? 'amber' : 'emerald'}>{yoloLoading ? 'Menghubungkan' : 'Siap'}</Badge>
        </div>
        <form onSubmit={submitYolo} className="mt-5 grid gap-4 lg:grid-cols-2">
          <label className="block">
            <span className="mb-2 block text-sm font-semibold text-slate-200">Backend YOLO URL</span>
            <input value={yoloForm.backendUrl || ''} onChange={(event) => updateYolo('backendUrl', event.target.value)} placeholder="http://127.0.0.1:5050" className="w-full rounded-2xl border border-white/10 bg-black/35 px-4 py-3 text-sm text-white outline-none ring-cyan-400 transition placeholder:text-slate-500 focus:ring-2" />
          </label>
          <label className="block">
          ...
          <label className="block lg:col-span-2">
            ...
            <input value={yoloForm.modelPath || ''} onChange={(event) => updateYolo('modelPath', event.target.value)} placeholder="models/best.pt" className="w-full rounded-2xl border border-white/10 bg-black/35 px-4 py-3 text-sm text-white outline-none ring-cyan-400 transition placeholder:text-slate-500 focus:ring-2" />
          </label>
          <div className="grid gap-4 lg:col-span-2 sm:grid-cols-4">
            <label className="block"><span className="mb-2 block text-sm font-semibold text-slate-200">Confidence</span><input value={yoloForm.confidence || ''} onChange={(event) => updateYolo('confidence', event.target.value)} placeholder="0.45" className="w-full rounded-2xl border border-white/10 bg-black/35 px-4 py-3 text-sm text-white outline-none ring-cyan-400 transition placeholder:text-slate-500 focus:ring-2" /></label>
            <label className="block"><span className="mb-2 block text-sm font-semibold text-slate-200">Image Size</span><input value={yoloForm.imageSize || ''} onChange={(event) => updateYolo('imageSize', event.target.value)} placeholder="320" className="w-full rounded-2xl border border-white/10 bg-black/35 px-4 py-3 text-sm text-white outline-none ring-cyan-400 transition placeholder:text-slate-500 focus:ring-2" /></label>
            <label className="block"><span className="mb-2 block text-sm font-semibold text-slate-200">Infer Every</span><input value={yoloForm.inferEvery || ''} onChange={(event) => updateYolo('inferEvery', event.target.value)} placeholder="1" className="w-full rounded-2xl border border-white/10 bg-black/35 px-4 py-3 text-sm text-white outline-none ring-cyan-400 transition placeholder:text-slate-500 focus:ring-2" /></label>
            <label className="block"><span className="mb-2 block text-sm font-semibold text-slate-200">JPEG Quality</span><input value={yoloForm.jpegQuality || ''} onChange={(event) => updateYolo('jpegQuality', event.target.value)} placeholder="85" className="w-full rounded-2xl border border-white/10 bg-black/35 px-4 py-3 text-sm text-white outline-none ring-cyan-400 transition placeholder:text-slate-500 focus:ring-2" /></label>
          </div>
          {yoloMessage && <div className="rounded-2xl border border-cyan-300/20 bg-cyan-300/10 px-4 py-3 text-sm font-semibold text-cyan-50 lg:col-span-2">{yoloMessage}</div>}
          <button type="submit" disabled={yoloLoading} className="rounded-2xl bg-cyan-500 px-4 py-3 text-sm font-black text-slate-950 hover:bg-cyan-400 disabled:cursor-not-allowed disabled:opacity-60 lg:col-span-2">{yoloLoading ? yoloLoadingLabel : yoloButtonLabel}</button>
        </form>
      </Card>
...
export default function IoTDashboardApp() {
  ...
  const [yoloConfig, setYoloConfig] = useState(() => normalizeYoloConfig(loadJson(YOLO_STORAGE_KEY, DEFAULT_YOLO_CONFIG)));
  const [yoloLoading, setYoloLoading] = useState(false);
  const [yoloCameraLoading, setYoloCameraLoading] = useState(false);
  const [yoloMessage, setYoloMessage] = useState('Isi IP stream kamera lalu jalankan sistem YOLO.');
  ...
  const startYoloBackend = async (normalizedConfig) => {
    const response = await fetch(yoloApiUrl(normalizedConfig.backendUrl, '/api/ppe/source'), {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        source: normalizedConfig.cameraSource,
        model_path: normalizedConfig.modelPath,
        conf: Number(normalizedConfig.confidence),
        imgsz: Number(normalizedConfig.imageSize),
        infer_every: Number(normalizedConfig.inferEvery),
        jpeg_quality: Number(normalizedConfig.jpegQuality),
        yolo_enabled: normalizedConfig.yoloEnabled,
      }),
    });
  ...
  const handleSaveYoloCamera = async (nextConfig) => {
    ...
    setYoloCameraLoading(true);
    const modeLabel = normalizedConfig.yoloEnabled ? 'sistem YOLO' : 'mode CCTV';
    setYoloMessage(`Menyimpan IP kamera dan menjalankan ${modeLabel}: ${normalizedConfig.cameraSource}...`);

    try {
      const payload = await startYoloBackend(normalizedConfig);
      setYoloMessage(payload.message || `IP kamera baru aktif di ${normalizedConfig.yoloEnabled ? 'Sistem YOLO' : 'Mode CCTV'}: ${normalizedConfig.cameraSource}`);
    } catch (error) {
      console.error('Gagal menjalankan kamera YOLO baru.', error);
      setYoloMessage('IP kamera tersimpan, tetapi backend YOLO belum merespons. Jalankan backend Python di port 5050 lalu tekan Simpan IP Kamera lagi.');
    } finally {
      setYoloCameraLoading(false);
    }
  };
  ...
  const handleSaveYoloConfig = async (nextConfig) => {
    ...
    setYoloLoading(true);
    const modeLabel = normalizedConfig.yoloEnabled ? 'sistem YOLO' : 'mode CCTV';
    setYoloMessage(`Menjalankan ${modeLabel} dengan kamera aktif: ${normalizedConfig.cameraSource}...`);

    try {
      const payload = await startYoloBackend(normalizedConfig);
      setYoloMessage(payload.message || `${normalizedConfig.yoloEnabled ? 'Sistem YOLO' : 'Mode CCTV'} berjalan dengan stream: ${normalizedConfig.cameraSource}`);
    } catch (error) {
      console.error('Gagal menjalankan sistem YOLO.', error);
      setYoloMessage('Konfigurasi tersimpan, tetapi backend YOLO belum merespons. Jalankan backend Python di port 5050 lalu tekan tombol lagi.');
    } finally {
  ...
  const renderPage = () => {
    ...
    switch (activePage) {
      ...
      case 'suhu': return <TemperaturePage data={temperature} canDownload={isCurrentAdmin} onDownload={() => downloadFullDataset('temperature')} />;
      case 'rfid': return <RfidPage data={rfid} canDownload={isCurrentAdmin} onDownload={() => downloadFullDataset('rfid')} />;
      case 'ats': return <AtsPage data={ats} canDownload={isCurrentAdmin} onDownload={() => downloadFullDataset('ats')} />;
      case 'yolo': return <Suspense fallback={<div className="grid min-h-[420px] place-items-center rounded-3xl border border-cyan-300/15 bg-slate-950 text-sm font-black text-cyan-100">Memuat section Deteksi YOLO...</div>}><YoloDetectionPage initialConfig={yoloConfig} canExportLogs={isCurrentAdmin} /></Suspense>;
      case 'users':
